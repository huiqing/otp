		   ...
		   ErlSpawnOpts* so) /* Options for spawn. */
{
    ...
    erts_get_default_trace_filter(&(p->trace_filter)); /*added by HL*/
    // p->trace_filter = NULL; /* added by HL */
    p->msg.first = NULL;
	...
	if (parent->trace_flags & F_TRACE_SOS) {
	    ...
	    p->tracer_proc = parent->tracer_proc;
            p->trace_filter = parent->trace_filter;  /* Added by HL */
	}
...
erts_debug_verify_clean_empty_process(Process* p)
{
    ...
    ASSERT(p->trace_filter == NULL); /* added by HL */
