...
/* added by HL */
Eterm
trace_filter(Process *p, Eterm pid_spec, Eterm Pattern)
{
  DeclareTmpHeap(mfa,3,p); /* Not really heap here, but might be when setting pattern */
  int matches =0;
  int i;
  ...
  /* else{
   ...
   /* TODO: add the case when pid_spec is port */
  if (is_pid(pid_spec)) {
    Process *tracee_p;
    erts_printf("Set filter for one process-------------------\n");
    tracee_p = erts_pid2proc(p, ERTS_PROC_LOCK_MAIN,
                             pid_spec, ERTS_PROC_LOCKS_ALL);
#ifdef ERTS_SMP
    if (tracee_p->scheduler_data==NULL)
      erts_printf("No schedular_data: \n");
    else
...
#ifdef ERTS_SMP
    if (tracee_p->scheduler_data==NULL)
      erts_printf("No schedular_data1\n");
    else
      erts_printf("Scheduler_data not null1\n");
#endif 
    matches =1;
  } else {
    if (pid_spec ==  am_all || pid_spec == am_existing){
      ...
      system_blocked = 1;
#endif
      erts_printf("Set filter for all/existing processes-------------------\n");
      erts_printf("Max Processes:%d\n", erts_max_processes);
      for (i = 0; i < erts_max_processes; i++) {
        Process* tracee_p = process_tab[i];
        if (! tracee_p) 
          continue;
        /*  if (tracee_p ->tracer_proc = NIL)
            continue;*/
    ...
    if (pid_spec == am_all || pid_spec == am_new){
      erts_printf("Set filter for all/new processes-------------------\n");
      erts_change_default_trace_filter(&match_prog_set);
    }
  }
 error:
 UnUseTmpHeap(3,p);
...
Eterm  
trace_trace_filter_2(BIF_ALIST_2)
{
  return trace_filter(BIF_P, BIF_ARG_1, BIF_ARG_2);
}
