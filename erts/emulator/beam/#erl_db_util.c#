...
{
    ErtsMatchPseudoProcess *mpsp;
    ErtsSchedulerData *sched_data;
#ifdef ERTS_SMP
    if (c_p->scheduler_data==NULL)
      erts_printf("scheduler data null\n");
    else
      erts_printf("Scheduler_data not null\n");     
    //  mpsp = (ErtsMatchPseudoProcess *) c_p->scheduler_data->match_pseudo_process;
    sched_data = erts_get_scheduler_data(), /* added by HL*/
    mpsp =(ErtsMatchPseudoProcess *) sched_data->match_pseudo_process; /* added by HL*/
    erts_printf("mpsp:%T", mpsp);
    if (mpsp)
      cleanup_match_pseudo_process(mpsp, 0);
    else {
      erts_printf("creat pseudo process\n");
...
Binary *erts_match_set_compile(Process *p, Eterm matchexpr) {
	...
	sz = size_object(matchexpr);
        erts_printf("sz: %d", sz);  /* added by HL*/
        erts_printf("match prog:%T\n", matchexpr);  /* added by HL*/
        erts_printf("Prog:%T\n", *prog);  /* added by HL*/
		    ...
		    Uint32 *return_flags)
{
    ...
    Uint save_op;
#endif /* DMC_DEBUG */
    erts_printf("entering db_prog_match %d\n", *pc); /* added by HL */
    ASSERT(base==NULL || HALFWORD_HEAP);

    erts_printf("enter db_prog_match: heap_size:%d\n", prog->heap_size);/* added by HL */
...
restart:
	...
	save_op = *pc;
    #endif
        erts_printf("Prog text:%d\n", *pc); /* added by HL */
        //  erts_printf("EP:%T\n", *ep);
	switch (*pc++) {
	case matchTryMeElse:
            erts_printf("MatchTryMeElse\n");
	    ASSERT(fail_label == -1);
	    fail_label = *pc++;
	    break;
	case matchArray: /* only when DCOMP_TRACE, is always first
			    instruction. */
            erts_printf("matchArray\n");
	    n = *pc++;
            erts_printf("Arity: %d\n", arity);
            erts_printf("n: %d\n", n);
	    if ((int) n != arity)
	...
	case matchPushT: /* *ep is a tuple of arity n, 
	    ...
	    *sp++ = tp + 1;
	    ...
	    break;
	case matchList:
          erts_printf("matchList\n");
	    if (!is_list(*ep))
		FAIL();
	    ep = list_val_rel(*ep,base);
	    break;
	case matchPushL:
          erts_printf("matchPushL\n");
	    if (!is_list(*ep))
		FAIL();
	    *sp++ = list_val_rel(*ep,base);
	    ...
	    break;
	case matchPop:
          erts_printf("matchPop\n");
	    ep = *(--sp);
	    break;
	case matchBind:
          erts_printf("matchBind\n");
	    n = *pc++;
	    variables[n].term = *ep++;
	    break;
	case matchCmp:
          erts_printf("matchCmp\n");
	    n = *pc++;
	...
	case matchPushC: /* Push constant */
          erts_printf("matchPuschC :%d, %d\n", in_flags, ERTS_PAM_COPY_RESULT);
